...
-- 3. Find all the tasks that are marked as done -- 
 SELECT task.description, status.id FROM task INNER JOIN status ON status.id = task.status_id WHERE status.name="done";
-- Result: 12 rows returned--

-- 4. Find all the tasks that are not marked as done--
 SELECT task.description, status.id FROM task INNER JOIN status ON status.id = task.status_id WHERE NOT status.name="done";
...
-- 10. Get the names of all statuses, sorted by the status with most tasks first--
 SELECT name, COUNT(task.status_id) AScount FROM status INNER JOIN task ON  task.status_id=status.id GROUP BY task.status_id ORDER BY count DESC;
